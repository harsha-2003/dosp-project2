...
-type child_specification(ETP) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(ETP)} |
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(ETV)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(ETV).
supervisor(Start) ->
...
-spec significant(child_specification(EUA), boolean()) -> child_specification(EUA).
...
-spec timeout(child_specification(EUD), integer()) -> child_specification(EUD).
timeout(Child, Ms) ->
...
-spec restart(child_specification(EUG), restart()) -> child_specification(EUG).
...
-spec map_data(child_specification(EUJ), fun((EUJ) -> EUL)) -> child_specification(EUL).
