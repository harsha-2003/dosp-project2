...
-spec map({ok, CPI} | {error, CPJ}, fun((CPI) -> CPM)) -> {ok, CPM} |
    {error, CPJ}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CPP} | {error, CPQ}, fun((CPQ) -> CPT)) -> {ok, CPP} |
    {error, CPT}.
...
-spec flatten({ok, {ok, CPW} | {error, CPX}} | {error, CPX}) -> {ok, CPW} |
    {error, CPX}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CQE} | {error, CQF}, fun((CQE) -> {ok, CQI} | {error, CQF})) -> {ok,
        CQI} |
    {error, CQF}.
'try'(Result, Fun) ->
    case Result of
...
-spec then({ok, CQN} | {error, CQO}, fun((CQN) -> {ok, CQR} | {error, CQO})) -> {ok,
        CQR} |
    {error, CQO}.
then(Result, Fun) ->
    'try'(Result, Fun).
...
-spec unwrap({ok, CQW} | {error, any()}, CQW) -> CQW.
...
-spec lazy_unwrap({ok, CRA} | {error, any()}, fun(() -> CRA)) -> CRA.
...
-spec unwrap_error({ok, any()} | {error, CRF}, CRF) -> CRF.
...
-spec unwrap_both({ok, CRI} | {error, CRI}) -> CRI.
unwrap_both(Result) ->
    case Result of
        {ok, A} ->
            A;
...
-spec 'or'({ok, CRL} | {error, CRM}, {ok, CRL} | {error, CRM}) -> {ok, CRL} |
    {error, CRM}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CRT} | {error, CRU}, fun(() -> {ok, CRT} | {error, CRU})) -> {ok,
        CRT} |
    {error, CRU}.
...
-spec all(list({ok, CSB} | {error, CSC})) -> {ok, list(CSB)} | {error, CSC}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 367).
-spec partition_loop(list({ok, CSQ} | {error, CSR}), list(CSQ), list(CSR)) -> {list(CSQ),
    list(CSR)}.
...
-spec partition(list({ok, CSJ} | {error, CSK})) -> {list(CSJ), list(CSK)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CSZ}, CTC) -> {ok, CTC} | {error, CSZ}.
...
-spec replace_error({ok, CTF} | {error, any()}, CTJ) -> {ok, CTF} | {error, CTJ}.
...
-spec values(list({ok, CTM} | {error, any()})) -> list(CTM).
values(Results) ->
