...
-type message(EAK) :: {message, EAK} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(EAL, EAM) :: {continue,
        EAL,
        gleam@option:option(gleam@erlang@process:selector(EAM))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(EAN, EAO) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        EAN,
        gleam@erlang@process:selector(message(EAO)),
        gleam@otp@system:debug_state(),
        fun((EAN, EAO) -> next(EAN, EAO))}.

-type started(EAP) :: {started, gleam@erlang@process:pid_(), EAP}.

-opaque initialised(EAQ, EAR, EAS) :: {initialised,
        EAQ,
        gleam@option:option(gleam@erlang@process:selector(EAR)),
        EAS}.

-opaque builder(EAT, EAU, EAV) :: {builder,
        fun((gleam@erlang@process:subject(EAU)) -> {ok,
                initialised(EAT, EAU, EAV)} |
            {error, binary()}),
        integer(),
        fun((EAT, EAU) -> next(EAT, EAU)),
        gleam@option:option(gleam@erlang@process:name(EAU))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(EAW) :: {ack, {ok, EAW} | {error, binary()}} |
...
-spec continue(EBB) -> next(EBB, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(EBN, EBO), gleam@erlang@process:selector(EBO)) -> next(EBN, EBO).
...
-spec initialised(EBU) -> initialised(EBU, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(EBZ, any(), ECB),
    gleam@erlang@process:selector(ECF)
) -> initialised(EBZ, ECF, ECB).
...
-spec returning(initialised(ECK, ECL, any()), ECQ) -> initialised(ECK, ECL, ECQ).
...
-spec new(ECU) -> builder(ECU, ECV, gleam@erlang@process:subject(ECV)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(EDA)) -> {ok, initialised(EDC, EDA, EDD)} |
        {error, binary()})
) -> builder(EDC, EDA, EDD).
...
-spec on_message(builder(EDM, EDN, EDO), fun((EDM, EDN) -> next(EDM, EDN))) -> builder(EDM, EDN, EDO).
...
-spec named(builder(EDX, EDY, EDZ), gleam@erlang@process:name(EDY)) -> builder(EDX, EDY, EDZ).
named(Builder, Name) ->
    {builder,
        erlang:element(2, Builder),
        erlang:element(3, Builder),
...
-spec select_system_messages(gleam@erlang@process:selector(message(EEM))) -> gleam@erlang@process:selector(message(EEM)).
...
-spec receive_message(self(any(), EEI)) -> message(EEI).
...
-spec initialise_actor(
    builder(any(), any(), EFF),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, EFF} | {error, binary()})
...
-spec start(builder(any(), any(), EFS)) -> {ok, started(EFS)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(EFZ), EFZ) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(EGB),
    integer(),
    fun((gleam@erlang@process:subject(EGD)) -> EGB)
) -> EGD.
